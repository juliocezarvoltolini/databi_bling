...
export class VendedorImportacao implements OnModuleInit {
    ...
    execute(contador: number, timeout: number = 1000): Observable<Vendedor | Vendedor[]> {
        try {
    ...
    private RemoverComissoes(vendedor: Vendedor): Observable<any> {
        ...
        if (comissoes.length > 0)
            return from(
                this.dataSource.manager.getRepository(VendedorComissao).delete(comissoes),
            );
        else return of(null);
    }
    ...
    private Salvar(vendedor: Vendedor): Observable<Vendedor> {
        return from(this.vendedorService.repository.save(vendedor)).pipe(
            catchError((err) => {
                logger.warn(`Erro ao persitir entidade Vendedor(NOME: ${vendedor?.pessoa.nome} / idOriginal:${vendedor.idOriginal}). Motivo: ${err.message}`);
                if (
    ...
    private mapearContatoParaPessoa(contato: IFindResponse): Observable<Vendedor> {
        const vendedor = new Vendedor();

        return from(this.dataSource
            .getRepository(Pessoa)
            .find({ where: { idOriginal: contato.data.id.toFixed(0) }, loadEagerRelations: false }))
