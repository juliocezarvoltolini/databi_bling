import { Between, ColumnType, DeepPartial, EntityMetadata, Equal, ILike } from 'typeorm';
...
export function BuildFindOptionsFromModel<T>(
  entity: DeepPartial<T>,
  entityMetadata: EntityMetadata,
) {
  let findOptions = {};
  const stringType: ColumnType[] = [
    'string',
    'char varying',
    'character varying',
    'char',
  ...
  entityMetadata.columns.forEach((column) => {
    ...
    if (Assigned(propertyValue)) {
      if (Assigned(column.referencedColumn))
        findOptions[propertyName] = BuildFindOptionsFromModel<T>(
          propertyValue,
          column.referencedColumn.entityMetadata,
        );
      else {
        if (stringType.includes(propertyType)) {
          //string
          findOptions[propertyName] = ILike(propertyValue);
        } else if (dateType.includes(propertyType)) {
          //date
          findOptions[propertyName] = Equal(propertyValue);
        } else if (Assigned(propertyValue['inicio']) && Assigned(propertyValue['fim'])) {
          //between date
